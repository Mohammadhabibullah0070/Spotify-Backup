 ...
 *   Final — Results report            (Milestone 13 ✅)
 */
...
export default function RestorePanel() {
  ...
  const { buildSummary } = useRestoreResults();

  const [showResults, setShowResults] = useState(false);
  const [results, setResults] = useState<RestoreResultSummary | null>(null);
  ...
  const step2Status: StepStatus = !playlistMap
    ? "locked"
    : restorer.status === "done"
      ? "done"
      : restorer.status === "restoring"
        ? "active"
        : "idle";
  ...
  const step3Status: StepStatus =
    restorer.status !== "done"
      ? "locked"
      : likedRestorer.status === "done"
        ? "done"
        : likedRestorer.status === "restoring"
          ? "active"
          : "idle";
  ...
  const restorerPct =
    restorer.status === "restoring" && restorer.progress
      ? restorer.progress.tracksTotal > 0
        ? Math.round(
            (restorer.progress.tracksAdded / restorer.progress.tracksTotal) *
              100,
          )
        : 0
      : restorer.status === "done"
  ...
  const likedPct =
    likedRestorer.status === "restoring" && likedRestorer.progress
      ? likedRestorer.progress.total > 0
        ? Math.round(
            (likedRestorer.progress.saved / likedRestorer.progress.total) * 100,
          )
        : 0
      : likedRestorer.status === "done"
        ? 100
  ...
  // ── All steps complete check ──────────────────────────────
  const allStepsDone =
    creator.status === "done" &&
    restorer.status === "done" &&
    likedRestorer.status === "done";

  // ── Build results when all steps are done ──────────────────
  if (allStepsDone && !results && source?.user) {
    const summary = buildSummary({
      playlistsCreated: creator.result?.created ?? 0,
      playlistsFailed: creator.result?.failed ?? 0,
      tracksRestored: restorer.result?.added ?? 0,
      tracksSkippedLocal: restorer.result?.skippedLocal ?? 0,
      tracksSkippedEpisode: restorer.result?.skippedEpisode ?? 0,
      tracksSkippedNull: restorer.result?.skippedNull ?? 0,
      tracksSkippedUnavailable: restorer.result?.attemptedUnavailable ?? 0,
      tracksFailed: restorer.result?.failed ?? 0,
      likedRestored: likedRestorer.result?.saved ?? 0,
      likedSkippedLocal: likedRestorer.result?.skippedLocal ?? 0,
      likedSkippedEpisode: likedRestorer.result?.skippedEpisode ?? 0,
      likedSkippedNull: likedRestorer.result?.skippedNull ?? 0,
      likedFailed: likedRestorer.result?.failed ?? 0,
    });
    if (summary) {
      setResults(summary);
      setShowResults(true);
  ...
  return (
    <div className="restore-panel">
      ...
      <header className="restore-panel__header">
        <span className="restore-panel__icon" aria-hidden="true">
          ♻️
        </span>
        <div>
      ...
      {/* ════════════════════════════════════════════════════════
          ...
          ════════════════════════════════════════════════════════ */}
      <section
        className="restore-panel__step-body"
        aria-label="Step 1: Create playlists"
      >
        ...
        {creator.status === "creating" && creator.progress && (
          <>
            <p className="restore-panel__progress-label">
              Creating playlist {creator.progress.current} of{" "}
              {creator.progress.total}:{" "}
        ...
        {/* Done */}
        {(creator.status === "done" ||
          (creator.status === "idle" && playlistMap)) &&
          creator.result && (
            <>
              <div className="restore-summary restore-summary--done">
                <SummaryRow
                  ...
                  value={creator.result.created}
                />
                {creator.result.skipped > 0 && (
                  <SummaryRow
                    icon="⏭"
                    label="Skipped"
                    value={creator.result.skipped}
                    muted
                  />
                )}
        ...
        {creator.status === "error" && (
          <ErrorBox
            message={creator.error ?? "Unknown error"}
            onRetry={() => creator.reset()}
          />
        )}
      </section>
      ...
      >
        <h3 className="restore-panel__step-heading">
          Step 2 — Add Tracks
          {!playlistMap && (
            <span className="restore-panel__locked-badge">
        ...
        {playlistMap && restorer.status === "idle" && (
          <>
            <div className="restore-summary">
              <SummaryRow
                icon="🎶"
                label="Total tracks in backup"
                value={backup.stats.totalPlaylistTracks}
              />
              <SummaryRow
                icon="⏭"
                label="Local files (will skip)"
                value={backup.stats.localFilesCount}
                muted
              />
              <SummaryRow
                icon="⏭"
                label="Podcast episodes (skip)"
                value={backup.stats.episodesCount}
                muted
              />
              <SummaryRow
                icon="⚠"
                label="Unavailable (will attempt)"
                value={backup.stats.unavailableCount}
                muted
              />
              <SummaryRow
                icon="⏭"
                label="Deleted tracks (skip)"
                value={backup.stats.nullTracksCount}
                muted
              />
            </div>
            <div className="restore-panel__limitations">
              <p className="restore-panel__limitations-title">
                ℹ️ How tracks are handled:
              </p>
              <ul>
                <li>✅ Normal tracks added using their Spotify URI</li>
                <li>
                  ✅ Duplicate URIs within a playlist are deduplicated
                  automatically
                </li>
                <li>
                  ⚠️ <strong>Running restore twice</strong> WILL create
                  duplicates on Spotify
                </li>
                <li>
                  ⚠️ Unavailable tracks: we try the linked URI first, then fall
                  back
                </li>
                <li>
                  ❌ Local files cannot be added via API — they are skipped
                </li>
                <li>
                  ❌ Podcast episodes cannot be added to music playlists —
                  skipped
                </li>
        ...
        {restorer.status === "restoring" && restorer.progress && (
          <>
            <p className="restore-panel__progress-label">
              Playlist {restorer.progress.playlistIndex} of{" "}
              {restorer.progress.playlistTotal}:{" "}
              <strong>{restorer.progress.playlistName}</strong>
            </p>
            <p className="restore-panel__progress-sub">
              Batch {restorer.progress.batchIndex} of{" "}
              {restorer.progress.batchTotal}
              {" — "}
              {restorer.progress.tracksAdded.toLocaleString()} tracks added so
              far
            </p>
          ...
          <>
            <div className="restore-summary restore-summary--done">
              <SummaryRow
                icon="✅"
                label="Tracks added"
                value={restorer.result.added}
              />
              {restorer.result.skippedLocal > 0 && (
                <SummaryRow
                  icon="💾"
                  label="Local files skipped"
                  value={restorer.result.skippedLocal}
                  muted
                />
              )}
              {restorer.result.skippedEpisode > 0 && (
                <SummaryRow
                  ...
                  value={restorer.result.skippedEpisode}
                  muted
                />
              )}
              {restorer.result.attemptedUnavailable > 0 && (
                <SummaryRow
                  icon="⚠"
                  label="Unavailable (attempted)"
                  value={restorer.result.attemptedUnavailable}
                  muted
                />
              )}
              {restorer.result.skippedNull > 0 && (
                <SummaryRow
                  icon="🗑"
                  label="Deleted tracks skipped"
                  value={restorer.result.skippedNull}
                  muted
                />
              )}
        ...
        {restorer.status === "error" && (
          <ErrorBox
            message={restorer.error ?? "Unknown error"}
            onRetry={() => restorer.reset()}
          />
        )}
      </section>
      ...
      >
        ...
        {restorer.status === "done" && likedRestorer.status === "idle" && (
          <>
            <div className="restore-summary">
              <SummaryRow
                icon="❤️"
                label="Liked songs in backup"
                value={backup.likedSongs.total}
              />
              <SummaryRow
                icon="⏭"
                label="Local files (will skip)"
            ...
            <div className="restore-panel__limitations">
              <p className="restore-panel__limitations-title">
                ℹ️ How liked songs are handled:
              </p>
              <ul>
                <li>
                  ✅ Tracks saved using <code>PUT /me/library</code> (Spotify
                  2026 API)
                </li>
                <li>
                  ✅ Processed in batches of 50 with rate-limit protection
                </li>
                <li>
                  ⚠️ Original "liked" order is <strong>not preserved</strong> —
                  Spotify always sorts by recently added
                </li>
                <li>
                  ⚠️ Running this twice will <strong>not</strong> create
                  duplicates (library saves are idempotent)
                </li>
                <li>❌ Local files skipped — no Spotify URI</li>
                <li>
                  ❌ Podcast episodes skipped — cannot be saved to track library
                </li>
              </ul>
            </div>
            <button
        ...
        {likedRestorer.status === "restoring" && likedRestorer.progress && (
          <>
            <p className="restore-panel__progress-label">
              Saving liked songs… batch {likedRestorer.progress.batch} of{" "}
              {likedRestorer.progress.batches}
            </p>
            <p className="restore-panel__progress-sub">
              {likedRestorer.progress.saved.toLocaleString()} of{" "}
              {likedRestorer.progress.total.toLocaleString()} saved
            </p>
        ...
        {likedRestorer.status === "done" && likedRestorer.result && (
          <>
            <div className="restore-summary restore-summary--done">
              <SummaryRow
                icon="✅"
                label="Liked songs saved"
                value={likedRestorer.result.saved}
              />
              {likedRestorer.result.skippedLocal > 0 && (
                <SummaryRow
                  icon="💾"
                  label="Local files skipped"
                  value={likedRestorer.result.skippedLocal}
                  muted
                />
              )}
              {likedRestorer.result.skippedEpisode > 0 && (
                <SummaryRow
                  icon="🎙"
                  label="Episodes skipped"
                  value={likedRestorer.result.skippedEpisode}
                  muted
                />
              )}
              {likedRestorer.result.skippedNull > 0 && (
                <SummaryRow
                  icon="🗑"
                  label="Deleted tracks skipped"
                  value={likedRestorer.result.skippedNull}
                  muted
                />
              )}
              {likedRestorer.result.failed > 0 && (
                <SummaryRow
                  icon="❌"
                  label="Failed"
                  value={likedRestorer.result.failed}
                  error
                />
              )}
            </div>
            ...
            <div className="restore-panel__done-note">
              ✅ All done! Liked songs restored to <strong>{destName}</strong>'s
              library.
            </div>
            <button
              className="restore-panel__btn restore-panel__btn--secondary"
        ...
        {likedRestorer.status === "error" && (
          <ErrorBox
            message={likedRestorer.error ?? "Unknown error"}
            onRetry={() => likedRestorer.reset()}
          />
        )}
      </section>
    </div>
...
function Step({
  ...
  return (
    <div
      className={`restore-step restore-step--${status}`}
...
function ProgressBar({ pct }: { pct: number }) {
  return (
    <>
      ...
      >
        <div
          className="restore-panel__bar-fill"
...
function ErrorBox({
  ...
  return (
    <div className="restore-panel__error">
      ...
      <p>{message}</p>
      <button className="restore-panel__btn" onClick={onRetry}>
        Try again
      </button>
    </div>
  );

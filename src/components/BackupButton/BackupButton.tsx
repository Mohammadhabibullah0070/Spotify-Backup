...
export default function BackupButton({
  ...
  if (status === "fetching" && progress) {
    ...
    const playlistPct =
      ((progress.playlistIndex - 1) / progress.playlistTotal) * 100;
    // Within current playlist (track fetch sub-progress)
    const trackPct =
      progress.tracksTotal > 0
        ? (progress.tracksFetched / progress.tracksTotal) *
          (1 / progress.playlistTotal) *
          100
        : 0;
    ...
    return (
      <div className="backup-panel">
        <div className="backup-panel__info">
          <span className="backup-panel__icon" aria-hidden="true">
            📦
          </span>
          <div className="backup-panel__text">
            <h2 className="backup-panel__title">Export Backup</h2>
            <p className="backup-panel__desc">
              Downloads a JSON file containing your liked songs and all playlist
              tracks — ready to restore later.
            </p>
            {isReady && (
              <ul className="backup-panel__preview">
                <li>
                  ❤️ <strong>{likedSongs.length.toLocaleString()}</strong> liked
                  songs
                </li>
                <li>
                ...
                <li>
                  📁 Local files and ⚠ unavailable tracks will be labelled in
                  the export
                </li>
              </ul>
            )}
  ...
  if (status === "fetching" || status === "building") {
    return (
      <div className="backup-panel backup-panel--working">
        ...
        >
          <div
            className="backup-panel__bar-fill"
  ...
  if (status === "error") {
    return (
      <div className="backup-panel backup-panel--error">
        ...
        <p>{error}</p>
        <button className="backup-panel__retry" onClick={reset}>
          Try again
        </button>
      </div>
    );
    ...
    return (
      <div className="backup-panel backup-panel--done">
        ...
        <div className="backup-panel__done-stats">
          ...
          <Stat
            label="Liked songs"
            value={result.totalLiked.toLocaleString()}
          />
          <Stat
            label="Playlists"
            value={result.totalPlaylists.toLocaleString()}
          />
          <Stat
            label="Playlist tracks"
            value={result.totalTracks.toLocaleString()}
          />
        </div>
        ...
        {result.warnings.length > 0 && (
          <details className="backup-panel__warnings">
            <summary>
              ⚠ {result.warnings.length} playlist
              {result.warnings.length > 1 ? "s" : ""} skipped
            </summary>
            <ul>
...
function Stat({
  ...
  return (
    <div className="backup-stat">
      ...
      <span
        className={`backup-stat__value${mono ? " backup-stat__value--mono" : ""}`}
      >
        {value}
      </span>
    </div>
